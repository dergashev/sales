...
> = {
  ...
  projectEvidenceGroups: {
    loading: { status: 'notApplicable', reason: SYNC_FIXTURE },
    empty: { status: 'implemented', where: 'a3-evgroup-empty — группа без ни одной находки говорит «Keine Anforderung dieser Art in den Quellen gefunden · nichts wird angenommen»: у здания с описанием и восемью чертежами действительно нет требований TGA, и честная выдача этого — состояние группы, а не подставленное значение по умолчанию' },
    partial: { status: 'implemented', where: 'a3-evgroup-count «n von m gezeigt» плюс a3-evgroup-disclosure «Alle N weiteren anzeigen»: голова группы — два-четыре пункта высокого влияния, остальное раскрывается внутри той же группы' },
    ready: { status: 'implemented', where: 'a3-evlist — каждый пункт со значением или требованием, областью действия, источником, полномочием и потребителями ниже по течению' },
    error: { status: 'notApplicable', reason: SYNC_FIXTURE },
    stale: { status: 'implemented', where: 'состояние пункта — вторая ось рядом с полномочием: stale и conflict приходят в слот freshness канонического AuthorityTrace, поэтому «нужна повторная проверка» видно, не теряя того, откуда значение взялось; оспариваемое значение показано РЯДОМ с подтверждённым (M-1/D-08), а не вместо него' },
    permission: { status: 'implemented', where: 'praesentation: раздел вместе со своей секцией не рендерится — состав источников, полномочия и внутренние потребители во клиентском профиле не существуют' },
  },
  projectSourceViewer: {
    loading: { status: 'notApplicable', reason: 'файл отдаёт бандлер по URL, полученному на сборке: промежутка загрузки данных перед появлением рамки не существует; собственную индикацию внутри PDF рисует сам просмотрщик браузера' },
    empty: { status: 'notApplicable', reason: 'просмотрщик открывается только для существующей записи регистра: документа без записи не существует, а запись без файла — это состояние unavailable ниже, а не пустота' },
    partial: { status: 'implemented', where: 'a3-docsrc[data-state=unavailable] со схематичной вкладкой: превью показывается с подписью «Schematische Vorschau · nicht das Quelldokument» — половина доказательства названа половиной, а не выдана за документ' },
    ready: { status: 'implemented', where: 'a3-docsrc[data-state=loaded]: имя файла, тип, ревизия, дата выпуска, «Seite n von m» и зитированная стелла — текстом в обвязке, плюс встроенный PDF' },
    error: { status: 'implemented', where: 'a3-docsrc-reason плюс a3-docsrc-recovery: причина названа («анализ не смог прочитать» и «файл не хранится» — два разных факта с разным восстановлением), рядом «Erneut laden» и «Zurück zum Projektverständnis»; рамка сохраняет высоту, поэтому «нет файла» и «нечего смотреть» не выглядят одинаково' },
    stale: { status: 'notApplicable', reason: 'документ — неизменяемая выдача с ревизией и датой: устареть относительно себя он не может, а замену прежней ревизии показывает происхождение записи (supersedes/duplicateOf) в самой строке регистра' },
    permission: { status: 'implemented', where: 'praesentation: регистр документов вместе с просмотрщиком не рендерится — источники подготовки в клиентский профиль не входят' },
  },
  guidedTour: {
