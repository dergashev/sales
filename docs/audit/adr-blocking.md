...
|---|---|---|
| `panel-right-width` | ~~400 px~~ **520 px — решено измерением, ADR снят** (TASK-15, 06.08): неразрывная строка максимального DE-тотала `≈ 10.489.000 €` при нормативных 64/68 требует 463,23 px + паддинги; host-инвариант DC-38 записан в README | ~~раздел 13 такой записи не содержит~~ закрыто вердиктом `codex-panel-primitives-260806.md` |
...
## 6b. Приписка 05.08.2026 (ночная сборка прототипа) — рабочие приближения внесены

Для четырёх записей раздела 6a в `tokens.css` внесены **рабочие приближения с
пометкой `[provisional]`** (правило проекта 2: нужного токена нет → добавить
provisional, не инлайнить). ADR-решения остаются открытыми — приближение не
является утверждением:

| Запись | Токен | Приближение | Откуда значение |
|---|---|---|---|
| 10 | `--size-toast-max-width` | `52ch` | ширина в `ch` по требованию самой записи (правило 3a) |
| 11 | `--motion-delay-hover-preview` | `200ms` | названо словами в README (DC-28); код читает токен, не константу |
| 13 | `--duration-toast-undo` | `8000ms` | названо словами в CLAUDE.md/README (DC-29) |
| — | `--size-popover-max-width` | `45ch` | новая заявка DC-21 (ADR-PENDING в контракте); `ch` по правилу 3a |

Записи 1–9, 12, 14–17 раздела 6a значений не получили: их компоненты в
прототипе не собраны либо значение требует решения владельца (например,
destructive-цвет, чей кандидат не проходит контраст, — придумать нельзя, R-25).
