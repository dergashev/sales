...
**Всего в остатке 57** · обёртка `tx()` хватит: 15 · целый ключ с числом: 13 · данные фикстур D-13: 3 · собрано в рантайме: 4 · вне словаря: 0 · ждут копирайта: 22
...
|---|---|---|
| 200 im Angebot: | src/screens/S3Konfigurator.tsx:1568 | целый ключ с подстановкой вместо частей · поставка помечает «да: номер KG» · есть `remainder5.coverage.kg200InOffer` на кусок — это мост, а не решение |
...
| Gebäude im Projekt | src/screens/OpportunityCard.tsx:362 | целый ключ с подстановкой вместо частей · поставка помечает «да: счётчик вне ключа» · есть `oppcard.buildingsInProject` на кусок — это мост, а не решение |
| Gebäude · | src/design-system/registry.tsx:213 · src/screens/OpportunityList.tsx:217 | целый ключ с подстановкой вместо частей · поставка помечает «да: count follows» · есть `remainder5.buildings.prefix` на кусок — это мост, а не решение |
...
| Total NUF nach DIN 277 | src/screens/OpportunityCard.tsx:371 | целый ключ с подстановкой вместо частей · поставка помечает «да: площадь вне ключа» · есть `oppcard.totalNufDin277` на кусок — это мост, а не решение |
| Total WFL nach WoFlV | src/screens/OpportunityCard.tsx:370 | целый ключ с подстановкой вместо частей · поставка помечает «да: площадь вне ключа» · есть `oppcard.totalWflWoflv` на кусок — это мост, а не решение |
...
| ≈ 85 % der BGF R+S | src/fixtures/derived-prototype.json:13 · src/screens/OpportunityCard.tsx:369 | целый ключ с подстановкой вместо частей · поставка помечает «да: процент» · есть `oppcard.bgfDerivedRatio` на кусок — это мост, а не решение |
...
| Der Gesamtbetrag bleibt unverändert — 70/22/8 verteilt, was bereits gerechnet ist. | src/screens/S3Konfigurator.tsx:1430 | обернуть в `tx()` · ключ `kg700.distributionUnchanged` |
| Leistungsbeginn ab OK Decke über UG | src/state/store.ts:3448 | обернуть в `tx()` · ключ `s3.basement.fromSlab` |
...
| Projektparameter bestätigt (Gebäude, Flächen, Einheiten) | src/state/store.ts:2685 | обернуть в `tx()` · ключ `domain4.journal.projectParametersConfirmed` |
...
| enthält die abgeleitete S-Fläche | src/screens/OpportunityCard.tsx:367 | обернуть в `tx()` · ключ `bldg.includesDerivedSArea` |
| nach HOAI und AHO | src/screens/S3Konfigurator.tsx:1425 · src/screens/S4Vergleich.tsx:132 · src/state/store.ts:1606 | обернуть в `tx()` · ключ `kg700.hoaiAhoMethod` |
| nach Planung | src/screens/S3Konfigurator.tsx:1354 | обернуть в `tx()` · ключ `remainder3.schedule.afterPlanning` |
| nicht Bestandteil | src/screens/S4Vergleich.tsx:118 · src/state/store.ts:3447 | обернуть в `tx()` · ключ `s4.cell.notIncluded` |
...
| vollständig inkl. Gründung | src/state/store.ts:3449 | обернуть в `tx()` · ключ `s3.basement.complete` |
