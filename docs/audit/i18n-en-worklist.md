...
### конкатенация с числом — нужен целый ключ (правило 36) — 17

| фрагмент | адрес | что делать |
|---|---|---|
| 200 im Angebot: | src/screens/S3Konfigurator.tsx:619 | целый ключ с подстановкой вместо частей · поставка помечает «да: номер KG» · есть `remainder5.coverage.kg200InOffer` на кусок — это мост, а не решение |
...
| Marge Eigenleistung nach Rabatt: | src/components/DiscountControl.tsx:104 | целый ключ с подстановкой вместо частей · поставка помечает «да: amount follows» · есть `remainder5.discount.inHouseMarginAfter` на кусок — это мост, а не решение |
...
| Schätzunsicherheit | src/components/ClientOutputGateDialog.tsx:101 · src/components/UncertaintyBand.tsx:7 · src/components/primitives.tsx:231 | целый ключ с подстановкой вместо частей · поставка помечает «да: percentage in cells» · есть `s4.row.uncertainty` на кусок — это мост, а не решение |
| Schätzunsicherheit ± | src/components/UncertaintyBand.tsx:7 · src/components/primitives.tsx:231 · src/screens/S5Export.tsx:82 | целый ключ с подстановкой вместо частей · поставка помечает «да: percentage follows» · есть `remainder5.uncertainty.prefix` на кусок — это мост, а не решение |
...
| Wege, Stellplaetze im Freien, Bepflanzung je m² BGF R ⚙ | — | целый ключ с подстановкой вместо частей · поставка помечает «да: area unit» · есть `remainder5.fixture.kg500ExternalWorksBasis` на кусок — это мост, а не решение |
...
| €/m² WFL nach WoFlV | src/components/OfferPanel.tsx:193 · src/engine/money.ts:163 · src/fixtures/demo-0001.json:224 | целый ключ с подстановкой вместо частей · поставка помечает «да: rate in cells» · есть `s4.row.wflRate` на кусок — это мост, а не решение |
| − 124.000 € € gegenüber Aufnahme | — | целый ключ с подстановкой вместо частей · поставка помечает «да: currency and amount» · есть `remainder5.delta.minus124ComparedWithInclusion` на кусок — это мост, а не решение |
| − 230.000 € € gegenüber Aufnahme | — | целый ключ с подстановкой вместо частей · поставка помечает «да: currency and amount» · есть `remainder5.delta.minus230ComparedWithInclusion` на кусок — это мост, а не решение |
...
| фрагмент | адрес | что делать |
|---|---|---|
| Beiträge · Summe = | src/components/OfferPanel.tsx:383 | обернуть в `tx()` · ключ `remainder5.offer.contributionsSum` |
| Der Gesamtbetrag bleibt unverändert — 70/22/8 verteilt, was bereits gerechnet ist. | src/screens/S3Konfigurator.tsx:511 | обернуть в `tx()` · ключ `kg700.distributionUnchanged` |
...
| Kundenansicht gesperrt: Klassifikation nach MBO | src/components/OfferPanel.tsx:608 | обернуть в `tx()` · ключ `remainder5.clientView.blockedMbo` |
| Leistungsbeginn ab OK Decke über UG | src/state/store.ts:1569 | обернуть в `tx()` · ключ `s3.basement.fromSlab` |
...
| Wohnfläche WFL nach WoFlV | src/screens/OpportunityCard.tsx:156 · src/screens/S2Vorbereitung.tsx:206 · src/screens/S3Konfigurator.tsx:303 | обернуть в `tx()` · ключ `s2.data.wflLabel` |
...
| aus Dokument | src/components/primitives.tsx:132 · src/screens/Grundlagen.tsx:146 · src/screens/OptionChapter.tsx:194 | обернуть в `tx()` · ключ `provenance.document` |
| bestätigt. | src/components/OfferPanel.tsx:609 · src/screens/ChapterBuildings.tsx:234 · src/screens/OpportunityCard.tsx:209 | обернуть в `tx()` · ключ `remainder5.status.confirmedSuffix` |
...
| nach HOAI und AHO | src/screens/S3Konfigurator.tsx:506 · src/screens/S4Vergleich.tsx:118 · src/state/store.ts:662 | обернуть в `tx()` · ключ `kg700.hoaiAhoMethod` |
| nach Planung | src/screens/S3Konfigurator.tsx:436 | обернуть в `tx()` · ключ `remainder3.schedule.afterPlanning` |
| nicht Bestandteil | src/screens/S4Vergleich.tsx:105 · src/state/store.ts:1568 | обернуть в `tx()` · ключ `s4.cell.notIncluded` |
| nicht enthalten | src/components/OfferPanel.tsx:65 · src/screens/S2Vorbereitung.tsx:417 · src/screens/S3Konfigurator.tsx:196 | обернуть в `tx()` · ключ `coverage.excluded` |
| nur Unterschiede | src/screens/S4Vergleich.tsx:149 | обернуть в `tx()` · ключ `s4.showDifferences` |
| vollständig inkl. Gründung | src/state/store.ts:1570 | обернуть в `tx()` · ключ `s3.basement.complete` |
| vom Kunden bestätigt | src/components/primitives.tsx:131 · src/screens/Grundlagen.tsx:147 · src/screens/S2Vorbereitung.tsx:173 | обернуть в `tx()` · ключ `provenance.customerConfirmed` |
