...
> **Две метки `SPEC` — разные оси, путать нельзя.** В реестре требований `SPEC` — это **основание замечания** по разделу 0.2 норматива: «проверяется в коде, а не по скриншоту». Здесь и в `output-model.md` §13 та же буква означает другое: **`SPEC` (не в прототипе)** — «прототип не реализует и не изображает».
>
> Разница практическая, а не терминологическая. Требование с основанием `SPEC` может быть полностью задано документацией и полностью обязательно к реализации — например конверт события журнала. Требование с пометкой «не в прототипе» недостижимо по построению — например настоящие хеши байтов файла, которого нет. Смешение этих двух осей давало ложный вывод, будто 110 требований объявлены закрытыми вопреки правилу: на деле артефакты их **задают**, а правило говорит лишь, что **доказать** их здесь нельзя.
>
> Формулировка «требуют кода, не документации» в плане ремедиации по этой причине неточна и читается как «не наша забота». Точнее: **специфицируются здесь, доказываются кодом.**
...
|---|---|---|
| **R-26** — десятичные месяцы | PO отклонил в части отображения; добавлено значение `halfMonthRounded` в `durationPresentationPolicy`. **Отклонение сужено (`decisions.md` D-17):** `halfMonthRounded` допустим только там, где день месяца у `startDate` и `endDate` не совпадает; при совпадении обязателен целый `n Monate`, потому что норматив запрещает не месяцы, а десятичный перевод | Рядом с длительностью всегда абсолютная дата завершения; политика и база входят в снапшот. Единая модель, единый epoch и точные даты не затронуты. CALC-007 не затронут: подпись `≈ 7,5 Monate` несёт префикс, потому что показ отличается от точной длительности |
