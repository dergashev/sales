...
KNOWN_OPEN_FILE = {
    ...
    #
    # ─── 06.08, поставка Codex по заданию № 07 ───────────────────────────
    # Файл — не вердикт, а СОДЕРЖИМОЕ: таблица переводов, из которой соберут
    # словарь продукта. Поэтому он проверяется полностью и НЕ подпадает под
    # исключение §2a для датированных вердиктов: то, что попадёт в продукт,
    # обязано соответствовать правилам продукта. Обе находки настоящие и
    # переданы автору файла заданием № 08 — зона записи не моя.
    ('docs/audit/verdicts/content/i18n-en-260806.md', 'R-18-LABEL'):
        'поставка копирайта воспроизводит усечённое «Zwischensumme» в немецкой '
        'колонке-источнике двух ключей — ровно ловушка, названная в задании № 07; '
        'правка у автора файла (Codex), передано заданием № 08',
    ('docs/audit/verdicts/content/i18n-en-260806.md', 'NBSP'):
        'поставка копирайта разделяет число и единицу обычным пробелом в двух '
        'местах; правка у автора файла (Codex), передано заданием № 08',
}
