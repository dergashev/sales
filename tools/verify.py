...
STALE_ARTIFACTS = {
    # Хеш переустановлен 08.09.2026: витрина изменена по тикету B1 —
    # снят специмен DC-43 «Interne Notiz» вместе с возможностью, и пример
    # примитива Textarea переведён на существующую возможность (письменное
    # обоснование ручного решения). Прежний хеш 35abf264… не совпадал с
    # файлом ещё до этой правки, то есть карантин уже НЕ действовал и снятие
    # ни одной находки не маскировало; запись восстановлена так, чтобы она
    # снова описывала тот файл, который лежит в дереве.
    'design-system/all3-design-system.html':
        ('48c59ed463d4a87dbfac49ee827d41fc3d1b8573dbb072d21cbe275940c3e545',
...
class Verifier:
    ...
    def _check_retired_paths(self):
        ...
        retired = (
            ...
            # Возможность «Interne Notiz» (DC-43) удалена целиком (требование
            # Product Owner № 8 аудита project-option-workflow-ca68ace).
            # Приватная заметка рядом с проектом клиента — второе,
            # неатрибутированное место коммерческой правды, поэтому запрет
            # держит не только имя файла: возврат под другим именем — тот же
            # запрещённый вход (правило проекта 34, README §2.5).
            ('InternalNote', re.compile(r'\bInternalNote\b'),
             '08.09.2026', 'приватной заметки в продукте не существует; '
             'возвращать её нельзя и под другим именем — политика полей '
             'NOTE-006/OUT-07 остаётся в output-model.md §5.4'),
        )
