...
KNOWN_OPEN = {
    ...
    # Текст задания описывает редакцию v3 и старел вместе с инструментом.
    # Отличается от вердикта Codex: тот пришпилен к commit и SHA-256 и потому
    # индексом не считается вовсе — измерение снимка обязано остаться прежним.
    ('docs/audit/verdicts/TASK-01-verify-mutations.md', 'CHECKS'):
        'задание описывает редакцию v3 (77 классов); правка — у автора задания',
    ('docs/audit/verdicts/TASK-01-verify-mutations.md', 'SELFTEST'):
        'задание описывает редакцию v3 (selftest 34/34); правка — у автора задания',
}
