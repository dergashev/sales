...
class Builder:
    ...
    def catalog(self):
        ...
        # из источника.
        # Коэффициент площадей S. Объявлен спецификацией §1 и до сих пор не
        # существовал в движке вовсе: `BuildingInput` знал одну надземную
        # площадь и множил её на полную ставку (решение D-26).
        f_s = re.search(r'f_S = ([\d,]+) — площади S', self.spec)
        if not f_s:
            raise Mismatch('каталог: f_S не найден в calculation-spec §1')
        ...
        return {
            ...
            # Доля ставки, по которой считаются площади S (§1, D-26).
            'fS': str(de(f_s.group(1))),
            'kgShares': {
