...
**Состояния взаимодействия:** `default / hover / focus / pressed`; `disabled` — **нативный**
на `Zurück` первой страницы и `Weiter` последней: у такого отказа нечего объяснять,
в отличие от гейта продукта (правило 12 сохраняется там, где отказ содержателен).
...
**Клавиатура:** обычный порядок Tab по кнопкам; активация Enter/Space нативная;
многоточие не фокусируемо. Зона нажатия 44 × 44 px через `.hit-target::before`.
...
утверждение о том, что действие относится к видимой странице.
