...
- *Контент переживает переход, но не дольше него.* React-компонент вправе
  ...
  корректным.
- *Приоритет слоя при перекрытии (EXP-02).* `.a3-preview`, `.a3-delta` и подпись
  полноты KG-таблицы («N von N Kostengruppen entschieden…», первый элемент под
  закреплённым блоком) — три претендента на одну и ту же зону над Level 2.
  Ровно один легален одновременно, порядок приоритета: превью > чип > подпись.
  Оверлейный вариант `.a3-preview-slot`/`.a3-delta-slot` внутри
  `.a3-change-slot-anchor` обязан быть НЕПРОЗРАЧНЫМ (`background`) — прозрачный
  слот пропускает подпись насквозь, что и было механизмом «трёх наложенных
  слоёв» в находке. Приоритет превью > чип закреплён численно
  (`.a3-preview-slot{z-index:2}` выше `.a3-delta-slot{z-index:1}`) —
  жизненный цикл выше гарантирует, что оба не остаются показанными
  ОДНОВРЕМЕННО дольше момента реакции на новое состояние, но у каждого свой
  independent CSS-переход (чип гаснет за `--motion-duration-fast`, превью
  появляется после собственной 200-мс задержки намерения) — без явного
  z-index более поздний по DOM chip рисовался поверх превью в это окно
...
3. **Осмотр не является побочным продуктом отказа.** Раскрытие деталей рендерилось
   ...
   подавляющее большинство — открыть было нельзя. `.a3-drow-inspect` /
   `.a3-drow-action-inspect` живут в самой строке и не зависят от `actions`.
