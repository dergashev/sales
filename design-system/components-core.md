...
**ACTION-001 · иерархия действия (аудит F11).** Три варианта различаются визуальным весом
не произвольно — у каждого экрана/блока-решения есть ровно одно действие, продвигающее
рабочий процесс вперёд, и оно **всегда** рендерится `primary` (сплошная заливка), без
исключений «потому что рядом уже есть другая primary-кнопка» или «потому что вариант
привычнее смотрится вторичным». `Zurück` / `Abbrechen` / `Rückgängig` и прочие
поддерживающие/отменяющие действия **никогда** не рендерятся `primary` — им принадлежит
`secondary` или `ghost`, что бы ни требовал локальный визуальный баланс экрана. На одном
экране/блоке решения не может быть двух `primary`-кнопок одновременно: если действий,
претендующих на «вперёд», больше одного, продуктовое решение о единственном must-have
действии принимает Product/Design, а не расстановка вариантов кнопки. Отклонение —
консьюмерский дефект конкретного экрана, а не повод менять эту таблицу.
...
не применяется — используется подписанный `<select>` / меню, а при открытом,
растущем списке — `Combobox` (LOCALE-004).

**Размеры:** `default` (видимая высота = `--size-hit-target-default`) ·
`compact` (видимая высота `--size-control-visual-sm`, 32 px). `compact`
разрешён ТОЛЬКО при одновременном выполнении обоих условий R-04: невидимая
зона `.hit-target::before` не перекрывает соседний контрол **и** контур
фокуса виден целиком. Практически это означает собственную минимальную
ШИРИНУ сегмента в 44 px — два сегмента высотой 32 px, но шириной 30 px дали
бы две перекрывающиеся зоны нажатия, то есть ровно тот дефект, ради запрета
которого условие существует.
...
`--control-height` · `--size-hit-target-default` · `--size-control-visual-sm` ·
`--space-1` · `--space-2` · `--space-3` · `--type-caption-size/-line` ·
...
использует для различимых глазом вариантов KG 300/400. Слот опционален: если `file`
отсутствует в манифесте или загрузка завершается ошибкой, весь слот скрывается без пустой
рамки, а текстовая карточка сохраняет размер hit target, выбор и последствия. Изображение
...
**Анатомия:** идентична RadioCardGroup, включая опциональный `mediaSlot` 4:3 и его
text-first fallback, но `<input type="checkbox">`, индикатор — квадрат с ✓,
и карточки независимы (нет группового цикла стрелок). Для DC-40 изображение остаётся
декоративным (`alt=""`) и никогда не заменяет заголовок, цену или состояние.
...
## 4. Раздел 7.3 — Forms, select, numeric input, slider и textarea
...
**Закрывает:** `LOCALE-004` · `FORM-001` · `LAYOUT-009`

**Назначение:** выбор одного значения из четырёх и более взаимоисключающих вариантов.
При двух–трёх значениях используется `SegmentedControl`; Tabs остаются только для
одновременно доступных панелей. Нативный `<select>` обязателен как базовая механика:
собственный listbox допустим только после отдельного контракта клавиатуры и screen reader.

**Анатомия:** `.a3-form-field` → видимый `<label for>` →
`<select class="a3-select-field">` → `helperText` → `[errorText]` →
`[disabledReason]`. Публичные классы: `.a3-select-field` и
`.a3-form-disabled-reason`; обвязка, helper и error наследуются от FormField.
Первая option является настоящим значением или явно названным состоянием данных, а не
пустым placeholder, который исчезает после выбора.

**Варианты:** `single` · `filter` · `locale`. `multiple` не входит в контракт: для
многозначного выбора применяется группа CheckboxCard с отдельной сводкой выбора.

*Взаимодействие*: `default` · `hover` (strong border) · `focus`
(двухслойный индикатор: внутренний белый separator и внешний focus ring) · `disabled`
(значение остаётся читаемым, причина видима и связана через `aria-describedby`) ·
`readOnly` — notApplicableReason: у нативного select нет корректного read-only режима;
если выбор нельзя менять, используется `disabled` с причиной либо статическое значение,
но не фальшивый `aria-readonly` · `pressed` — нативное раскрытое меню, визуальный стиль
остаётся за платформой · `loading` — select отключён, root несёт `aria-busy="true"`,
а причина называет загрузку вариантов.

*Данные*:

- `ready` — список полон, текущее значение допустимо и выбрано;
- `loading` — список ещё загружается: control отключён, текст `Wird geladen` видим;
- `empty` — допустимых вариантов нет: control отключён, единственная строка
  `Keine Optionen verfügbar`, рядом названа причина пустоты;
- `partial` — показана неполная выборка, helper называет границу (`3 von 5 Quellen`),
  выбор разрешён только если частичность не меняет множество допустимых значений;
- `stale` — сохранённое значение показано вместе со `Stand <Datum>` и действием обновления;
- `error` — причина, последствие и retry находятся рядом; последнее подтверждённое
  значение не подменяется первым option;
- `permission` — текущее значение видно, select отключён, причина называет требуемое право.

**Длинное значение и локализация (`LOCALE-004`).** Control занимает доступную ширину
FormField (`width: 100%; min-width: 0; max-width: 100%`). Строка FR/ES до +35 % не
расширяет grid или панель: однострочное выбранное значение обрезается визуально через
ellipsis, а полное значение остаётся в доступном имени и в нативном списке option.
Ширину нельзя задавать по немецкой строке или фиксировать в px. В narrow layout control
занимает всю строку.

**Клавиатура и touch:** зона select не меньше `--size-hit-target-default` = 44 px;
`Tab` переводит фокус, `Alt+↓`/`Space` открывают нативный список, стрелки меняют option,
`Esc` закрывает его. Глобальные одноклавишные shortcuts внутри select не срабатывают
(`KEY-001`). Нативное меню не заменяется набором div/button.

**Screen reader:** имя берётся только из видимого `<label>`; helper, error и причина
disabled перечислены в `aria-describedby`; `aria-invalid="true"` ставится при ошибке,
`aria-busy="true"` — на FormField при loading. Число вариантов не дублируется вручную:
его сообщает нативная платформа.

**Токены:** токены FormField + `--size-hit-target-default` ·
`--color-action-disabled-bg/-text/-border` · `--color-focus-ring` ·
`--color-focus-separator` · `--border-width-focus`.

**Запреты:**
- placeholder вместо видимого label — `FORM-001`;
- SegmentedControl с четырьмя и более значениями — `LOCALE-004`;
- `opacity` как единственный disabled-носитель и disabled без видимой причины;
- ширина по текущей немецкой строке, горизонтальный скролл формы или обрезка доступного имени;
- собственный listbox без полного keyboard/screen-reader контракта —
  **канонический ответ на эту потребность существует и называется
  `Combobox` (ниже)**; частная реализация запрещена именно потому, что
  контракт теперь есть.

---

### Combobox

**Закрывает:** выбор ОДНОГО значения из списка, который сужается набором
текста (применяются `LOCALE-004`, `STATE-003`, `R-04`, `FORM-001`)

**Зачем отдельный контракт.** `SegmentedControl` заканчивается на трёх
значениях (`LOCALE-004`), `Select` показывает весь список сразу и не
сужается. Открытый список, который растёт вместе с данными — страна, город,
ответственный в реестре проектов, — не покрывается ни тем, ни другим, и
именно на этом месте раньше появлялись частные `role="listbox"` без
клавиатурного контракта (прямой запрет в `Select`). Контракт закрывает эту
дыру ОДНИМ каноническим контролом, а не разрешением на частный.

**Анатомия:** `.a3-form-field.a3-combobox` → видимый `label[for]` →
`.a3-combobox-shell` ( `input[role="combobox"]` + `.a3-combobox-toggle` ) →
`ul[role="listbox"]` → `li[role="option"]` ( `.a3-combobox-option-mark` +
подпись ) → `[role="status"]` (число найденного) → `[helperText]` →
`[причина/ошибка/устаревание/неполнота]`.

Роль `combobox` стоит НА `input`, не на обёртке (ARIA 1.2, editable
combobox); список — отдельный `listbox`, названный `«{label} · Vorschläge»`,
а не именем самого поля: два узла с одинаковым доступным именем делают
запрос «контрол по имени Stadt» неоднозначным.

**Варианты:** `single-select`. Множественный выбор этим контрактом НЕ
закрывается: он требует другой семантики выбранного (чипы, снятие,
объявление количества) и получит собственный контракт.

**Состояния:**

*Взаимодействие*: `default` · `hover` · `focus` (кольцо на всей оболочке,
чтобы кнопка-раскрытие не выглядела отдельным контролом) · `open`
(`aria-expanded="true"`) · `active-option` (`aria-activedescendant`
указывает на пункт ПО ПОЗИЦИИ; адресация по значению даёт `id` с пробелом —
недопустимый IDREF, и объявление молча перестаёт работать) · `selected`
(`aria-selected` + видимая галочка, не только заливка) · `disabled` — только
вместе с видимой причиной.

*Данные*: `ready` — список получен, значение выбрано, поле показывает его
подпись · `loading` — поле недоступно, `aria-busy` на обёртке; список
источника ещё не пришёл · `empty` — источник пуст: «нечего выбирать», и это
НЕ «ничего не найдено» (набор ни при чём) · `partial` — список заведомо
неполон, сказано словами рядом · `stale` — список получен, но источник с тех
пор изменился · `error` — источник не загрузился: **выбранное значение
остаётся видимым**, поле помечено `aria-invalid`, рядом причина и повтор ·
`permission` — недоступно по правам, причина рядом. Четыре последних
состояния объявляются КАЖДОЕ своей строкой: право доступа, возраст списка и
его полнота — разные факты, и склейка сообщила бы только первый.

**Клавиатура:** `Tab` входит в поле и НЕ раскрывает список (форма, которая
раскрывается на каждой остановке, кричит на клавиатуру). `↓` раскрывает и
переводит активность на следующий пункт, `↑` — на предыдущий, оба
закольцованы. `Home` / `End` — первый / последний. Набор символов сужает
список и ставит активность на первый совпавший. `Enter` фиксирует активный
пункт, закрывает список и ОСТАВЛЯЕТ фокус в поле. `Escape` закрывает список
и возвращает подпись выбранного значения; закрытый `Escape` не перехватывает
(контракт оверлеев: закрывается ближайший слой, а не все сразу). `Tab`
закрывает список без фиксации.

**Screen reader:** `label[for]` даёт имя полю; `role="combobox"` +
`aria-expanded` + `aria-controls` + `aria-autocomplete="list"` описывают
всплывающий список; `aria-activedescendant` называет активный пункт, поэтому
фокус не покидает поле и набор не прерывается. Число найденного объявляется
в `role="status" aria-live="polite"` С ЗАДЕРЖКОЙ, после того как набор
устоялся: живая область, срабатывающая на каждую букву, — шум, а не
доступность. Кнопка-раскрытие имеет настоящее доступное имя и стоит вне
Tab-порядка (`tabindex="-1"`); `aria-hidden` на кликабельном контроле
запрещён.

**Токены:** `--border-contrast-1px` · `--border-hairline` ·
`--color-surface-default` · `--color-surface-selected` ·
`--color-selection-border` · `--color-focus-ring` · `--border-width-focus` ·
`--color-text-primary` · `--color-text-secondary` · `--color-text-muted` ·
`--color-action-disabled-bg` · `--color-action-disabled-text` ·
`--control-height` · `--size-hit-target-default` · `--size-icon-sm` ·
`--size-status-accent-width` · `--type-input-size` · `--type-small-size/-line`
· `--space-2` · `--space-3` · `--layer-popover`.

**Запреты:**
- `placeholder` вместо видимого label — `FORM-001`;
- роль `combobox` на обёртке вместо `input`;
- одинаковое доступное имя у поля и у его списка;
- `id` пункта, собранный из значения (пробел в IDREF);
- объявление числа найденного на каждое нажатие клавиши;
- показ незафиксированного запроса как выбранного значения после ухода
  фокуса или `Escape`;
- потеря выбранного значения при ошибке загрузки списка;
- `aria-hidden` на кликабельной кнопке-раскрытии;
- раскрытие списка по одному лишь получению фокуса.
...
**Варианты:** `emailBody` · `comment`.
Вариант `note` (внутренняя заметка) снят 08.09.2026 вместе с возможностью
DC-43: приватной заметки в продукте не существует, и возвращать её нельзя
под другим именем (правило проекта 34). Требования `NOTE-001/003/004/007`
ниже остаются — они о видимой подписи, о предупреждении потери черновика и
о запрете молчаливой перезаписи более свежей версии, то есть о самом
`Textarea` с автосохранением, а не о снятой возможности.
...
**TABLE-002.** Числовые колонки — `text-align: right` через класс `.numeric`; выравнивание
по десятичному разделителю. *[F04, 2026-08-21] `--font-numeric` больше не активирует `tnum`/
`lnum`: в установленном файле Visuelt Pro фича `tnum` расширяет `.`/`,` до полной цифровой
ширины (цифры расходятся пробелами вокруг `.`/`,`), что и есть сам дефект F04 — проверено напрямую в браузере на
живом рендере (`font-variant-numeric: tabular-nums` даёт идентичный артефакт, `lnum` разницы
не делает). Выравнивание колонки обеспечивает `text-align: right`; табличная ширина цифр
до правки шрифта недостижима без визуального дефекта. Токен сохранён (не удалён), чтобы не
ломать остальные ссылки на `--font-numeric` в этом файле и в README.md — ADR по замене файла
шрифта остаётся открытым, вне периметра этого тикета.* Единица стоит **в заголовке
...
## 7. Раздел 7.6 — Feedback, toast, dialog, save status и состояния данных
...
**Hit-testing.** Регион-хост всегда `pointer-events:none`; только карточка в видимом
состоянии `.a3-show` восстанавливает `pointer-events:auto`. При начале exit действия внутри
сразу отключаются и карточка становится `aria-hidden`, а удаление из DOM происходит после
анимации. `opacity:0` само по себе не делает ни карточку, ни пустой fixed-хост прозрачными
...
- невидимая карточка или пустой fixed-хост, перехватывающие указатель;
...
**Закрывает:** STATE-001 · STATE-003 · STATE-005 · STATE-006 · STATE-008 · R-16 · R-17 ·
R-18 · gate 7 · gate 10

**Назначение:** общий статичный носитель пяти явно названных неготовых состояний данных:
`empty` · `partial` · `error` · `stale` · `permission`. Компонент задаёт геометрию,
типографику и порядок объяснения, но не подменяет доменный контракт владельца данных.
`loading` показывает DC-35 Skeleton в известной конечной геометрии, а `ready` показывает сам
готовый компонент. Отдельного декоративного блока «готово» не существует: он дублировал бы
данные статусом и смешивал две оси (`STATE-001`).

**Публичные классы (ровно шесть):** обязательный root `.a3-data-state` и пять модификаторов
`.a3-data-state-empty` · `.a3-data-state-partial` · `.a3-data-state-error` ·
`.a3-data-state-stale` · `.a3-data-state-permission`. Root без модификатора запрещён: по нему
невозможно понять, какое утверждение делает блок. `empty / partial / error` — основные
взаимоисключающие состояния; `stale` и `permission` — независимые оси, но их сочетание с
основным состоянием допускается только когда оба утверждения истинны и названы текстом.
`empty + permission` запрещено: «данных нет» и «данные есть, но недоступны роли» — разные
взаимоисключающие факты.

**Анатомия:** `root` → `sign [aria-hidden=true]` → `stateSentence` (видимо называет состояние)
→ `[detail]` → `[action]`. Знак не заменяет текст (gate 7). Действие — настоящий Button или
Link по своему контракту; root не получает `tabindex`, click-handler или роль контрола.

**Варианты и обязательное содержание:**

- `empty` — законное отсутствие данных. Для полноценного EmptyState root дополнительно несёт
  `.a3-empty-spec` и соблюдает DC-24: объяснение назначения + ровно одно действие; read-only
  профиль оставляет объяснение без действия;
- `partial` — называет, что уже известно, чего не хватает и как это квалифицирует результат.
  Неизвестная цена/площадь не превращается в `0` (R-18);
- `error` — четыре части `cause → impact → remedy → retry policy` (`STATE-005`). Красный бордер
  вторичен; причина и средство остаются текстом `--color-text-primary`;
- `stale` — называет прежний stand/версию или абсолютную дату, затронутый результат и способ
  обновления. Устаревшее не выглядит актуальным и не выдаётся клиенту как свежее;
- `permission` — называет доступную роли причину и следующий шаг получения доступа. Если
  профиль не вправе знать о самом объекте, блок отсутствует в DOM/дереве/ответе по R-17;
  этот вариант не является способом раскрыть скрытое поле.

*Взаимодействие*: `default` — обычный блок в потоке · `hover / focus / pressed` —
только у вложенного Button/Link · `selected / disabled / readOnly / loading` —
notApplicableReason: root не является контролом, не выбирается и не запускает процесс;
read-only влияет на наличие действия, но не делает текст состояния серым или недоступным.

*Данные*: `empty / partial / error / stale / permission` — пять модификаторов выше ·
`loading` — notApplicableReason: пока результат неизвестен, утверждать пустоту, ошибку или
неполноту преждевременно; владелец показывает DC-35 и несёт `aria-busy=true` · `ready` —
notApplicableReason: актуальные полные данные показываются самим компонентом-владельцем;
отдельный status-block создал бы ложную шестую поверхность вместо содержания.

**Переходы:** ими владеет data-компонент: `loading → ready | empty | partial | error`,
`ready | partial → stale`, `permission → loading` после получения доступа. DataStateBlock не
хранит состояние и не запускает переход сам. **События:** собственных не создаёт; вложенное
действие создаёт событие своего доменного контракта.

**Клавиатура:** root отсутствует в Tab-order. Фокусируются только вложенные Button/Link;
порядок следует DOM: объяснение читается до действия. Размер и focus ring действия — R-04/R-03.

**Screen reader:** статический блок остаётся обычным текстом. `role=status`/`aria-live=polite`
добавляется владельцем только при динамическом результате операции; статический specimen в
галерее live-region не получает (`XSC-17`). `role=alert` допустим только для новой немедленной
блокирующей ошибки, не для каждой ошибки при загрузке страницы. Декоративный знак скрыт через
`aria-hidden=true`; видимое предложение произносит состояние словами.

**Токены:** `--color-surface-default` · `--color-surface-subtle` ·
`--color-text-primary` · `--color-text-secondary` · `--border-default` ·
`--border-warning` · `--border-error` · `--type-body-size/-line/-weight` ·
`--type-small-size/-line/-weight` · `--space-1` · `--space-3`.

**Запреты:** root без модификатора · состояние только цветом/знаком · `0` вместо неизвестного ·
`empty` вместо `loading/error/permission` · ошибка без любой из четырёх частей STATE-005 ·
stale без stand/даты и способа обновления · permission как CSS-скрытие защищённых данных ·
автоматический live-region на статическом образце · `.numeric` на всём блоке вместо только
числового фрагмента.
...
### 9.1 Готовность 24 контрактов к сборке · 10.08.2026
...
До TASK-20 файл содержал 22 заголовка примитивов: `Skeleton` не является отдельным
контрактом (это доменный DC-35), а перечень задания № 11 пропустил реальные
`CheckboxCard` и `Toast`. TASK-20 добавил отсутствовавший самостоятельный контракт
`Select`, TASK-30 — `DataStateBlock`; актуальное число — 24.
...
| Select | готов | `.a3-select-field`, disabled-reason и long-value образцы в витрине |
...
| DataStateBlock | готов | `.a3-data-state` + пять явных модификаторов; `loading` остаётся DC-35, `ready` — обычным компонентом |
...
Итого: **18 готовы**, **6 ждут владельца**. Новые имена только добавлены; существующие
`a3-*` не переименованы.
...
## 13. Раздел офферной рельсы · SIDEBAR 03 (backlog `2be8e69c`, SB-21)

Четыре контракта ниже закрывают ровно тот пробел, который аудит 2026-08-24 назвал причиной
собственного P0 SIDEBAR 01: `.a3-rail-sticky-top`, `.a3-preview-slot` (переименован из
`.a3-ghost-slot`, SB-30) и `.a3-delta-slot` уже жили как канонические классы в
`components.css`, но не имели контракта здесь — поэтому никто не был обязан задать высотный
бюджет закреплённого блока до того, как его отсутствие стало измеримым дефектом. Все четыре
паттерна ниже уже реализованы и выпущены (SIDEBAR 01 и SIDEBAR 02, `OfferPanel.tsx`) — это документирование
существующего source of truth, а не новая реализация; смотри §0 «дизайн-система — источник
компонентов, а не справочник».

### OfferRailShell · `<aside class="a3-rail">`

**Назначение:** постоянная правая панель коммерческого контекста Konfigurator/Vergleich/
Export/Einstellungen — «цена видна всегда» (product decision, не декоративная панель).
Один инстанс на экран, ровно один independent scroll owner внутри рельсы.

**Анатомия:** `aside.a3-rail[aria-label]` → `h2.a3-visually-hidden` (rail root heading,
SB-29 — landmark `aria-label` не заменяет heading-навигацию) → `div.a3-rail-sticky-top`
(sticky-контекст, без собственного контента) → `div.a3-rail-header-budget` (Level 1,
см. StickyCommercialBand) → Level 2/3 контент в нормальном потоке рельсы. `<aside>` не
вложен в другой scroll-контейнер; документ и `<main>` скроллятся независимо (SB-19: «одна
рельса — один scroll owner», 4 независимых контекста сведены к 3 на странице).

**Геометрия:** ширина `clamp()`-ограничена в диапазоне 440–520 px (`--panel-right-width`),
рабочая область (`<main>`) остаётся ≥ 560 px при 1280 × 800. `scroll-padding-top` рельсы
равен `--size-rail-header-budget` (тот же токен, что ограничивает закреплённый блок) — так
`scrollIntoView`/фокус на строке Level 2/3 никогда не приземляется под закреплённым блоком
(SB-28). На Variantenvergleich/Export/Einstellungen рендерится только Level 1 (`variant=
"level1"`), геометрия рельсы не меняется (rule 38).

**Состояния взаимодействия:** root не является контролом — состояния принадлежат детям
(disclosure-кнопкам Level 2/3, `Details`/`Herkunft anzeigen` триггерам).

**Состояния данных:** `empty` (Declared Pricing Scope пуст — идентичная формулировка на
Level 1 и Level 2, rules 16 и 30) · `partial` (`priceUnavailable`, заголовок суммы меняется на
`Zwischensumme der kalkulierten Positionen`) · `ready` · `error` не самостоятелен для root —
принадлежит владельцу расчёта выше по дереву · `stale` не применяется (рельса всегда читает
текущую проекцию) · `permission` = клиентский профиль (`s.mode === 'praesentation'`):
building-атрибуция, `⚙`-детали внутреннего происхождения и KG 800-разбивка гасятся на уровне
данных (R-25, `clientProjection.ts`), не CSS.

**Клавиатура и screen reader:** landmark `aria-label` + новый `h2` дают и роль, и позицию в
heading-списке. Не более одного `h2` на рельсу; дочерние секции — `h3`, без пропуска уровня
(SB-29). `aria-live="polite"` живёт ТОЛЬКО на дочернем ChangeAnnouncer (см.
StickyCommercialBand), не на root и не на `.a3-rail-sticky-top` (SB-17).

**Токены:** `--panel-right-width` · `--size-rail-header-budget` · `--color-surface-default` ·
`--color-border-strong`.

**Запреты:** второй scroll-контейнер внутри рельсы (SB-19) · `aria-live` на весь root или на
`.a3-rail-sticky-top` (SB-17) · рендер полного Level 2/3 breakdown на нерабочих экранах
(Variantenvergleich/Export/Einstellungen, SB-20) · rail-local копия примитивов ниже.

### StickyCommercialBand · `.a3-rail-sticky-top > .a3-rail-header-budget`

**Назначение:** закреплённый коммерческий заголовок — сумма, диапазон, ведущая ставка,
срок, короткое объявление изменения. Единственный элемент рельсы, который остаётся видимым
при скролле её содержимого.

**Анатомия:** `.a3-rail-sticky-top{position:sticky;top:0}` (без собственной высоты/padding —
только stacking-контекст и якорь для оверлея слотов) → `.a3-rail-header-budget{max-height:
var(--size-rail-header-budget);overflow:hidden}` (весь Level 1 контент: `.a3-mtag`
scope-tag → `.a3-heroband` из трёх `.a3-hb` героев DC-38) → `.a3-change-slot-anchor{height:0}`
(нулевой sibling ПОСЛЕ budget-блока, несёт `.a3-preview-slot`/`.a3-delta-slot` как оверлей —
не резервирует высоту внутри закреплённого блока, SB-02) → ChangeAnnouncer (`aria-live=
"polite"`, визуально скрытый, ровно два значения: изменившаяся сумма + подпись изменения,
SB-17).

**Геометрия — обязательный численный бюджет (сам факт его отсутствия был причиной SB-01).**
`--size-rail-header-budget` обязана вмещать три героя DC-38 (64, 48 и 48 px) с их подписями и
диапазоном при НАИБОЛЕЕ плотной измеренной комбинации (2+ здания, полный DIN-276 scope,
1280 × 800) без клиппинга. Деградационная лестница (реализована `OfferPanel.tsx`,
`degradeLevel` 0–4) — единственный механизм, которому разрешено превышать текущий контент
budget: шаг 1 гасит триггеры `Herkunft anzeigen`; шаг 2 переводит диапазон в `compact`
представление DC-3; шаг 3 гасит secondary-rate/per-unit строку; шаг 4 (последний резерв)
гасит scope-tag, но НИКОГДА срок/`Fertigstellung`. Контракт не разрешает захватывать
дополнительную высоту сверх токена — новый контент внутри budget обязан либо участвовать в
деградационной лестнице, либо доказать бюджет заново измерением на 1280 × 800.

**Состояния взаимодействия:** `.a3-preview` (DC-28 Geist-Vorschau, переименован из
`.a3-ghost`, SB-30) — `default` невидим (`opacity:0`), `hover/focus` на опции показывает
превью с задержкой (`--motion-delay-hover-preview`) через `.a3-show` · `.a3-delta` (DC-2
Delta-Chip) — появляется на 8 с после commit-события, тот же `.a3-show` контракт. Оба
mutually exclusive во времени и делят один `.a3-change-slot-anchor`.

**Состояния данных:** `loading` не применим (проекция синхронна) · `empty`/`partial`/`ready`
наследуются от родителя (см. OfferRailShell) · `error` не самостоятелен · `stale` не
применяется · `permission` — `.a3-preview` гейтится на рендере под `s.mode==='intern'` (не
только на setter'е, чтобы смена режима во время открытого превью не оставляла его видимым
клиенту).

**Клавиатура и screen reader:** три `Herkunft anzeigen`-триггера несут разные accessible
name (сумма/ставка/срок), не «Herkunft anzeigen» трижды (F-35). ChangeAnnouncer — единственный
`aria-live="polite"` источник в рельсе; его текст строится из тех же уже локализованных
значений, что видимый герой рендерит (никогда собственное чтение DOM).

**Токены:** `--size-rail-header-budget` · `--size-ghost-row` · `--size-delta-row` ·
`--type-display-numeric-desktop-size/-line` · `--type-display-numeric-narrow-size/-line` ·
`--color-brand-accent` · `--motion-delay-hover-preview` · `--motion-reveal`.

**Запреты:** резервирование `--size-ghost-row`/`--size-delta-row` ВНУТРИ закреплённого
блока (только оверлей через `.a3-change-slot-anchor`, SB-02) · рост контента без деградации
или без нового измерения бюджета · `aria-live` на что-либо шире суммы+изменения (SB-17).

**Жизненный цикл `.a3-preview` (AUD-01, находки EXP-01/EXP-02/EXP-14) — контракт
дополнен, геометрия и `mutually exclusive` выше не переоткрываются.** Аудит
2026-08-25 нашёл, что геометрический контракт выше описывает ГДЕ живёт превью, но
не КОГДА оно обязано погаснуть — регрессия того же класса, что F-03 (закрыт
ранее), в новой межопционной форме.

- *Существование.* Превью существует, только пока курсор/фокус — на НЕвыбранном
  варианте АКТИВНОЙ Option. Наведение на уже выбранный вариант активной Option не
  показывает чужой (предыдущий) призрак — либо ничего, либо `±0`.
- *Триггеры гашения — каждый обязан гасить `s.preview` СИНХРОННО в самом действии,
  не полагаясь на то, что CSS-переход сам «когда-нибудь» скроет контент:* уход
  курсора/фокуса · фиксация (клик) · смена активной Option (включая undo/redo
  создания Option — тот же класс перехода, что обычное переключение) · смена главы
  конфигуратора · смена `pipelineView` (маршрута). Смена режима (`intern`/
  `praesentation`) сознательно НЕ входит в этот список — она гейтится на рендере,
  а не на сеттере (см. «Состояния данных» выше), и это решение не переоткрывается.
- *Раздельная хореография входа/выхода.* Вход (наведение на новый вариант) и выход
  (любой триггер гашения) — РАЗНЫЕ переходы, не зеркала друг друга: вход несёт
  `--motion-delay-hover-preview` (DC-28, задержка распознавания намерения), выход —
  без задержки, `--motion-duration-fast` (правило 20, «выход = fade 120 мс»),
  и обязан ЗАВЕРШИТЬСЯ до начала `useCountUp` героя при фиксации (часть волны
  правила 19: превью гаснет → герой считает → чип долетает → интервал). Общий
  `transition` на входном/выходном состоянии, из которого выход наследует входную
  задержку, — реализационный дефект этого класса, не альтернативная трактовка
  контракта.
- *Контент переживает переход, но не дольше него.* React-компонент вправе
  латчить последнее показанное значение на время визуального ухода (тот же
  приём, что у `.a3-delta` — элемент постоянен, содержимое не гаснет пустым), но
  латч обязан САМ разрешиться в `null` по истечении своего же выходного окна —
  латч, который держит значение бессрочно и полагается только на CSS-класс для
  видимости, превращает любой пропущенный триггер гашения в незаметный (пока
  видимость держится корректно) риск, который EXP-01 показал не всегда
  корректным.
- *Приоритет слоя при перекрытии (EXP-02, развёрнут 16.09.2026).* `.a3-preview`,
  `.a3-delta` и подпись полноты KG-таблицы («N von N Kostengruppen
  entschieden…», первый элемент под закреплённым блоком) — три претендента на
  одну и ту же зону над Level 2. Оверлейный слот обязан быть НЕПРОЗРАЧНЫМ
  (`background`) — прозрачный слот пропускает нижележащее насквозь, что и было
  механизмом «трёх наложенных слоёв» в находке.
  **Порядок приоритета: результат > превью > подпись**, и превью с чипом больше
  не взаимоисключающи во времени. Прежняя редакция ставила превью выше чипа и
  требовала, чтобы одновременно был легален ровно один; закреплялось это тем,
  что начало превью гасило `activeDelta`. Следствие называется одним
  предложением: наведя мышь на соседний вариант, читатель СТИРАЛ результат
  собственного клика — то есть жест осмотра уничтожал ответ, который читатель
  только что получил и ещё не дочитал. Показ последствия не может отменять
  показ факта. Поэтому чип живёт своё окно DC-2 независимо от указателя, слот
  кокпита (`.a3-cockpit-changeslot`) — один grid-слой с `isolation:isolate`,
  результат лежит на `--layer-change-result` поверх превью на
  `--layer-change-preview` и НЕПРОЗРАЧЕН, а превью, пока чип жив, гасится
  целиком (`:has(.a3-cockpit-change)`), чтобы из-под двухстрочного чипа не
  торчал хвост трёхстрочного превью. Оба по-прежнему живут в одном
  зарезервированном слоте фиксированной высоты — правило 24 требует, чтобы ни
  одно из четырёх состояний (пусто / превью / чип / оба) не двигало вёрстку.
  Числовой приоритет остаётся обязательным по той же причине, что и в находке:
  без явного z-index более поздний по DOM элемент рисуется поверх в окне
  передачи (живая находка, `elementFromPoint` на точном кандидате).
  *Классы `.a3-change-slot-anchor` / `.a3-preview-slot` / `.a3-delta-slot`,
  которые нормировала прежняя редакция, в `src/**` больше не используются:
  живой слот один, и это `.a3-cockpit-changeslot`. Их правила в CSS оставлены
  и нормой этого пункта больше не являются.*
- *Источник подписи.* Подпись превью/чипа/анонсера — тот же человеческий текст,
  что уже показывает исходный контрол (карточка опции), через тот же каталог/
  словарь, а не отдельно выведенная строка. Сырой `groupId`/`value`/enum —
  запрещённое состояние подписи для ЛЮБОГО вида решения, не только для
  инстанса, найденного аудитом.
- *Шов закреплённого блока (EXP-14) — DESIGN SYSTEM CONSTRAINT, не закрыто
  этой партией.* Обычная прокрутка колёсиком (в отличие от `scrollIntoView`/
  клавиатурного фокуса, уже компенсированных `scroll-padding-top`, SB-28)
  может остановиться в любой позиции — строка, рассечённая нижним краем
  закреплённого блока пополам, читается как обрыв. Аудит рекомендовал
  «декоративное затухание» на границе; реализация такого затухания через
  `linear-gradient` (единственный обычный CSS-приём для мягкого края)
  прямо нарушает правило 4 («никаких градиентов… плоская иерархия: бордеры
  и воздух») и была отклонена `GOV-TOKEN`-проверкой `tools/verify.py` —
  подтверждено этой же партией, не гипотетически. Существующий
  `.a3-rail-header-budget{border-bottom:var(--border-hairline)}` остаётся
  единственной канонической границей шва — она соответствует языку
  системы, но не решает конкретную жалобу «рассечённый глиф». Требуется
  ЛИБО отдельное продуктовое решение принять жёсткий край как часть
  плоского языка системы, ЛИБО новый non-gradient канонический приём
  (например `scroll-snap` на строках Level 2, чтобы резонансная позиция
  прокрутки не могла остановиться посередине строки) — вне бюджета этой
  партии, возвращено как находка, не домыслено втихую.

### CompositionRow · `<tr class="a3-expand"> + <tr class="a3-kg-child">`

**Назначение:** одна строка группы DIN 276 (Level 2, «Kostenzusammensetzung») с
раскрывающимся списком собственных вкладов-решений — единственный список композиции оффера
(заменяет исторически раздельные recap и Kostentreiber-таблицу, SIDEBAR 01 SB-03).

**Анатомия:** `tr.a3-expand[.a3-open]` → `td` (twist-button `.a3-twistbtn` либо статичная
подпись группы, non-colour-only маркер изменения `.a3-tag.a3-blue` при наличии) → `td.a3-num`
× 2 (сумма, доля). При раскрытии — sibling `tr.a3-kg-child` → `ul` → `li` на каждый вклад:
подпись (через `translatedDriverLabel`, ключевая структурная трансляция, НЕ reverse-lookup
строки) + опциональная building-атрибуция (только `s.mode==='intern'`, R-25) + подписанная
сумма (`signed()`/локализованная обёртка).

**Клиентский агрегационный контракт (SB-13):** в `s.mode==='praesentation'`
building-атрибуция гасится по R-25 — если это оставить единственным изменением, два вклада
одной опции у разных зданий становятся дублирующей строкой без различия. Контракт требует:
строки с идентичной переведённой подписью объединяются в одну, сумма — точная сумма Decimal
(ассоциативна, инвариант правила 32 не затрагивается). В `s.mode==='intern'` строки остаются
раздельными, с building-атрибуцией, как раньше.

**Геометрия:** без вложенного скролла — раскрытие увеличивает высоту секции Level 2 в
нормальном потоке рельсы (не budget-блока).

**Состояния взаимодействия:** `.a3-twistbtn` — `default` свёрнут · `aria-expanded` меняется
по клику/Space/Enter · `focus-visible` — двухслойный outline · нет `hover`-only смысла.

**Состояния данных:** `empty` (группа без строк не рендерится) · `partial` (`rowUnpriced` —
`Preis nicht ermittelt` вместо `0 €`, rule 16/SB-06) · `ready` · `error` не самостоятелен ·
`stale` не применяется · `permission` = агрегация выше.

**Клавиатура и screen reader:** twist-кнопка — настоящий `<button>` с `aria-expanded`/
`aria-controls`; дочерний `<ul>` получает `id`, на который ссылается `aria-controls`.

**Токены:** `--color-status-info`(blue tag) · `--color-border-subtle` · типографика таблицы
из §5 (Tables и disclosure).

**Запреты:** дублирование одного вклада в двух списках (recap + drivers, устранено SB-03) ·
экспонирование building-идентификатора в клиентском профиле (R-25, не переоткрывается) ·
`0 €` без причины для включённой группы без ценовой базы (rule 16).

### RailTable · `<table class="a3-kg">` / `<table class="a3-driver-table">`

**Назначение:** табличное представление DIN-276 разбивки (`.a3-kg`) и Kostentreiber-детализации
(`.a3-driver-table`) — числовые колонки, независимое округление строк и процентов
(SB-32), одна нормативная сигнатура для округления (`--color-text-secondary`, НЕ
`--color-text-muted` при этом кегле — см. ниже).

**Анатомия:** `div.a3-tbl-scroll{overflow-x:auto}` → `table` → `caption` (`.a3-visually-hidden`
либо `sr-only` — оба варианта закрыты этим контрактом как эквивалентные: видимый заголовок
секции уже называет таблицу, caption существует для навигации по таблицам, не для
дублирования видимого текста) → `tbody` → `tr` → `td.a3-num`/`th[scope=row]`.

**Геометрия:** таблица не имеет `min-width` — рендерится строго в ширину хоста
(канонический фикс SB-18-соседнего кластера F-35: `.a3-driver-table{min-width}`/
`.a3-bar-cell{min-width}` удалены на источнике); `.a3-tbl-scroll` остаётся guard-контейнером
для непредвиденно широкого контента, не средством, оправдывающим фиксированную ширину.

**Округление (rounding disclosure) — минимальный размер токена, обязателен к соблюдению.**
`--color-text-muted` (`#8C8C8C`) даёт **3,36 : 1** на белой поверхности при кегле `14px
regular` — ниже AA (SB-18). Правило: `--color-text-muted` **запрещён для текста мельче
24px regular / 18,67px Bold** (то же пороговое правило, что ограничивает бренд-оранжевый,
правило 5); ниже этого порога — только `--color-text-secondary` (`#6B6B6B` на `#FFFFFF`,
измерено `tools/verify.py` по формуле WCAG как 5,33 : 1 — инструмент авторитетен; более раннее
прозаическое измерение аудита 2026-08-24 расходилось с этой величиной и не переносится). Все
существующие rounding-note инстансы, включая `offer.kgTable.roundingNote`, приведены к
`--color-text-secondary`.

**Состояния взаимодействия:** `Details`-триггеры (`OriginPopover`, `triggerLabel="Details"`)
наследуют `.a3-hk-val` контракт целиком, включая расширенную 44×44 hit/focus-зону через
`::before` (SB-31). Плотная строка `.a3-driver-table` — R-04 условие «зона не перекрывает
соседей» требует эмпирической проверки при текущем межстрочном интервале (известный класс
дефекта LAYOUT-009 для компактных таблиц, не закрыт этим контрактом полностью — см. §11
открытые пробелы).

**Состояния данных:** те же семь, что RailTable's owner (CompositionRow/OfferRailShell) — таблица
сама не владеет `empty`/`loading`; `partial`-строка (`rowUnpriced`) заменяет `0 €`.

**Клавиатура и screen reader:** каждый `Details`-триггер получает уникальное accessible name
(`Details · <переведённая подпись драйвера>` + опциональная building-атрибуция) — не
переиспользует один и тот же видимый текст «Details» как единственное имя (F-35, T04-AC8, не
регрессирует).

**Токены:** `--color-text-secondary` · `--size-hit-target-default` · типографика `.a3-num`
(`tabular-nums`).

**Запреты:** `--color-text-muted` на тексте мельче порога правила 5 · `min-width` на
`.a3-driver-table`/`.a3-bar-cell` (канонически удалено, не восстанавливается) · дублирующее
accessible name на двух и более `Details`-триггерах в одной таблице.

## 14. Раздел Documents-Workspace · принятый аудит 05.09.2026

Источник: `docs/audit/documents-workspace-ux-audit-328330c.md` (статус ACCEPTED
TARGET). Раздел добавляет **две канонические возможности** и фиксирует **эволюцию
двух существующих**. Продуктовая композиция (сетка `.a3-docws`, компактная шапка
проекта, доменная сводка анализа, порядок полей документа) остаётся у продукта и
контрактом системы не является.

### WorkflowNavigator · `.a3-wfn`

**Назначение:** иерархическая навигация по маршруту проекта: шесть сгруппированных
станций верхнего уровня вместо тринадцати равноправных строк. Группировка —
**презентация**: маршруты, предусловия, гейты и сохранённое состояние workflow не
меняются ни одной строкой.

**Анатомия:** `nav.a3-wfn[aria-label]` → `ol.a3-wfn-list` →
`li.a3-wfn-stage.a3-wfn-{current|done|upcoming|locked}` →
(`button.a3-wfn-button.hit-target` | `div.a3-wfn-static`) →
`span.a3-wfn-marker[aria-hidden]` + `span.a3-wfn-text` (`.a3-wfn-label` +
`.a3-wfn-state`); при `state=current` дополнительно `ol.a3-wfn-sub` →
`li.a3-wfn-substep` → (`button.a3-wfn-subbutton.hit-target` |
`span.a3-wfn-substatic`).

**Прогрессивное раскрытие — контракт, а не флаг.** Вложенные станции рендерятся
**только** у текущей станции. Булева пропа, которая позволила бы потребителю снова
опубликовать всё дерево, отсутствует намеренно: именно этот дефект раздел и
закрывает.

**Геометрия:** одна горизонтальная полоса шириной `--measure-workspace`, нижний
бордер `--border-hairline`; маркер `--space-5` × `--space-5` с `--border-hairline`;
подпись — роль Label 14/20 Medium, состояние — Caption 12/16 Regular
`--color-text-muted`; текущая
станция получает `--color-brand-accent` нижней границей 2 px и Bold-подпись. При
≤1439 px горизонтальные отступы станций сжимаются до `--space-2`; полоса **никогда**
не прокручивается горизонтально в поддерживаемом диапазоне.

**Состояния взаимодействия:** `default / hover / focus / pressed` — у станций с
`onSelect`. `disabled` не используется: недостижимая станция не является кнопкой,
а `locked` **всегда** называет причину (правило 12). `selected` выражается через
`aria-current="step"` ровно у одной станции.

**Состояния данных:** `loading / empty / partial / ready / error / stale /
permission` неприменимы: навигатор не загружает и не интерпретирует данные владельца,
он отображает состояние маршрута, которое владелец уже вычислил. Это named
notApplicableReason правила 30.

**Клавиатура:** roving tabindex по станциям верхнего уровня — ровно одна станция в
цикле Tab; `ArrowLeft/ArrowRight/ArrowUp/ArrowDown` перемещают фокус, `Home`/`End` —
на края; стрелка **не активирует** станцию (STEP-003). Вложенные станции — обычные
кнопки в естественном порядке Tab, вне roving-цикла. Зона нажатия и фокуса 44 × 44 px
через `.hit-target::before` (R-04).

**Screen reader:** `nav` с обязательным `aria-label`; `ol` несёт последовательность;
`aria-current="step"` — только у текущей; маркер `aria-hidden`, потому что состояние
всегда написано словом (`aktuell / erledigt / ausstehend / gesperrt`), никогда цветом
или формой в одиночку (правило 8).

**Токены:** `--measure-workspace` · `--space-1/-2/-3/-5` · `--color-brand-accent` ·
`--color-text-primary/-secondary/-muted` · `--border-hairline` ·
`--border-width-strong` · `--type-small-size/-line` · `--type-caption-size/-line` ·
`--font-weight-medium/-bold`.

**Запреты:** второй владелец `WorkflowNavigator` вне
`src/design-system/WorkflowNavigator.tsx` (машинно: GOV-CAPABILITY) · раскрытие
вложенных станций у не-текущей станции · `locked` без причины · накопление булевых
режимных проп вместо явных состояний · повтор слова «blockiert» у каждой будущей
станции.

### Pagination · `.a3-pgn`

**Назначение:** канонический постраничный контрол реестра. До этого раздела канонической
пагинации в системе не существовало, и любой длинный реестр рос страницей вместо того,
чтобы листаться.

**Анатомия:** `nav.a3-pgn[aria-label]` → `p.a3-pgn-range` +
`div.a3-pgn-controls` (`button.a3-pgn-step.hit-target` +
`ol.a3-pgn-pages` → `li.a3-pgn-page` → `button.a3-pgn-number.hit-target` |
`li.a3-pgn-gap` → `span[aria-hidden]` + `button.a3-pgn-step.hit-target`).

**Модель страниц:** ≤ 7 страниц — перечислены все; > 7 — первая · последняя ·
текущая ± 1 с многоточиями. Многоточие **никогда** не заменяет ровно одну страницу:
такая замена стоит клика и не экономит ничего. Чистые функции `pageCountFor`,
`clampPage`, `paginationRange`, `paginationPages` экспортируются, чтобы владелец
выводил ту же модель без рендера, а сама модель проверялась юнит-тестом.

**Границы владения:** контрол **управляемый** (`page`/`pageCount` приходят от
владельца) и не режет список, не грузит данные и не владеет live-областью: смена
страницы значима и должна объявляться, но контрол, который объявляет сам себя,
конкурирует с единственной сдержанной live-областью рабочей области. `onPageChange` —
хук: объявляет и переводит фокус владелец.

**Геометрия:** `--space-3` сверху; кнопки `--space-2`/`--space-3` внутренних отступов,
`--border-contrast-1px`, минимальная ширина `--size-control-visual-sm`; текущая
страница — `--color-surface-selected` + `--color-selection-border` + Bold; числа —
`tabular-nums`.

**Состояния взаимодействия:** `default / hover / focus / pressed`; на границах
(`Zurück` первой страницы, `Weiter` последней) — **`aria-disabled`, не нативный
`disabled`**: кнопка остаётся в DOM и в порядке Tab и при этом действительно
инертна. Клавиатурный читатель, у которого `Zurück` исчезает на первой странице и
появляется на второй, вынужден заново узнавать форму контрола на каждой странице;
граница, которая всегда на месте и просто отказывается двигаться, — устойчивый
ориентир. Та же семантика, что у канонической `Button` для любого заблокированного
действия, — теперь эти два контракта не расходятся. *(Первая редакция контракта
требовала нативный `disabled`; изменено принятым контрактом реестра Projects
от 05.09.2026, который называет это требование дважды и объясняет причину. Правило
12 сохраняется: отказ границы не нуждается в объяснении, поэтому `disabledReason`
здесь нет.)* `selected` — `aria-current="page"`.

**Состояния данных:** семь осей неприменимы — контрол отображает позицию в наборе,
владелец которого сам объявляет `loading / empty / partial / ready / error / stale /
permission`. Named notApplicableReason правила 30.

**Клавиатура:** обычный порядок Tab по кнопкам, **включая заблокированные границы**;
активация Enter/Space нативная; многоточие не фокусируемо. Зона нажатия 44 × 44 px
через `.hit-target::before`.

**Screen reader:** `nav` с обязательным `aria-label`; у каждой числовой кнопки
доступное имя вида «Seite N»; `aria-current="page"` — у текущей; многоточие
`aria-hidden`, потому что страницы, которые оно замещает, достижимы соседними
числами.

**Токены:** `--space-1/-2/-3` · `--border-contrast-1px` · `--color-surface-default` ·
`--color-surface-subtle` · `--color-surface-selected` · `--color-selection-border` ·
`--color-text-primary/-secondary/-disabled/-muted` · `--size-control-visual-sm` ·
`--type-small-size/-line`.

**Потребители:** `src/screens/ProjectHome.tsx` (реестр документов проекта) ·
`src/screens/OpportunityList.tsx` (реестр Projects). Оба — 10 на страницу, контрол
с 11-го результата: **один продукт — один ритм пагинации**. Читатель, выучивший
10/страницу в Документах, не должен встретить 12/страницу в Проектах.

**Запреты:** локальная копия пагинации у потребителя (машинно: GOV-CAPABILITY) ·
десятки числовых кнопок без ограничения · собственная live-область · любое
утверждение о том, что действие относится к видимой странице · разный размер
страницы у разных потребителей.

### Изменение контракта 17.09.2026: причина в `freshness` у AuthorityTrace

`AuthorityTrace.freshness` получил необязательный `staleReason`: было
`{ staleReason: string }`, стало `{ staleReason?: string }`. Слот делает два
утверждения — переводит сведение в состояние `stale` (и тем самым печатает
знак «veraltet») **и** выводит фразу, объясняющую, почему. Разделены они
потому, что в объединённом чек-листе `v4` фраза была третьей формулировкой
одной и той же вещи: знак состояния, эта фраза и строка под ней, которая
называет конкурирующую **величину** («Eine neuere Quelle nennt 48 · der
bestätigte Wert gilt, bis jemand entscheidet»). Последняя говорит по делу,
фраза пересказывала её прозой.

**Обязательно:** убрать можно только причину. Не передавать `freshness`
вовсе — другое утверждение: «сведение актуально», и знак состояния тогда
показывает уровень доверия (`bestätigt`) у величины, которая оспорена. Это
и есть тот случай, когда состояние передаётся одной полосой на краю —
запрещено правилом 8.

**Потребитель без собственной строки причины передаёт `staleReason`.**
`v1`–`v3` её не имеют и передают.

### Изменение контракта 16.09.2026: квалификатор подписи в SegmentedControl

`Segment` получил необязательное поле `detail?: string`. Рендер выводит его
второй строкой внутри той же подписи (`.a3-segment-text` > `.a3-segment-detail`),
на кегле caption и во вторичном цвете; при отсутствии поля разметка сегмента
не меняется ни на узел.

Почему поле, а не вклейка в `label`. Квалификатор кандидата («neuere Quelle»,
«ersetzt», «ältere Quelle») в одной строке со значением читается как часть
самого значения: «19.710 m² · ältere Quelle» выглядит как одна величина из
четырёх слов, и различие, ради которого квалификатор существует, пропадает
ровно там, где принимается решение. Второго контрола для этого не заведено по
той же причине, что и в двух изменениях выше: сегментированный выбор в системе
один, и «сегменты с пояснением» были бы его копией, расходящейся на следующей
правке.

Границы: `detail` — РАЗРЯД ПОДПИСИ, а не статус. Статус в сегмент не
переносится: правило 8 требует, чтобы состояние несли знак и слово, и
приглушённая строка внутри радиокнопки этого обеспечить не может. Ширина не
фиксируется (правило 36) — длинный перевод переносится внутри сегмента.

### Изменение контракта 16.09.2026: раскрытие в DataTable и примитив Checkbox

Два изменения канонических источников, оба сделаны ради одной поверхности
(Schnittstellenmatrix, шаг `Schnittstellen & Verantwortung` в `v4`) и оба
записаны здесь, потому что источник компонентов один и продуктовой копии не
существует.

1. **`DataTable` · `row.disclosure` и `row.depth`.** Строка может раскрывать
   строки под собой: `disclosure: { expanded, onToggle }` рендерит имя строки
   настоящей `<button aria-expanded>` (`.a3-dt-twist`, цель нажатия 44 px через
   `hit-target`), `depth` задаёт отступ дерева. Таблица **остаётся без
   состояния**: потребитель решает, что раскрыто, и передаёт только видимые
   сейчас строки — это по-прежнему не data grid. Обоснование: список, который
   нельзя свернуть, — не другая таблица, а эта же со всеми раскрытыми
   разделами; для двухсот строк это скролл, а не чтение. Альтернатива —
   вторая таблица в продукте — запрещена правилом «компонент существует в
   одном экземпляре».
2. **`Checkbox` — новый примитив** (`src/components/controls.tsx`,
   `.a3-check`). Голый помеченный бокс для ячейки длинного списка. Не заменяет
   `CheckboxCard`: та — плитка с заголовком, следствием и описанием, выбираемая
   из небольшой сетки; здесь вопрос не «что из этого», а «эта позиция — да или
   нет», трижды в строке, и видимое имя уже есть у строки и колонки. Поэтому
   `label` по умолчанию — **скрытое доступное имя** (`sr-only`), а не отсутствующее:
   чекбокс, опознаваемый только по положению в сетке, для скринридера не опознаваем.
   Цель нажатия — 44 px через `hit-target`; потребитель обязан держать соседние
   боксы не ближе 44 px (условие R-04, а не пожелание) — в матрице это делает
   `.a3-matrix-table td{min-width:var(--size-hit-target-default)}`.

### Эволюция: ProcessingJob и DocumentRow

Аудит нашёл в этой паре три неправды и одну недостающую компоновку. Все четыре
закрыты **канонически**, а не обходом на одном экране:

1. **`READY` — состояние.** Документ, который никогда не отправляли, показывал
   `QUEUED` и нулевую полосу прогресса, то есть утверждал, что обработка принята и
   вот-вот начнётся. `QUEUED` теперь означает ровно то, что написано — принято и ждёт
   очереди, — а `READY` несёт правду до старта и **не имеет** прогресс-обработки
   вообще. `.a3-drow-ready` + тихий `.a3-drow-quiet-state` вместо статус-марки: реестр
   из восьми одинаковых марок ничего не выделяет, акцент остаётся за состояниями,
   требующими решения.
2. **Словарь задания — словарь анализа.** `ProcessingJobState` =
   `READY | CANCELLED | RUNNING | COMPLETE | COMPLETE_WITH_ISSUES`. Терминальное
   задание с открытыми исходами больше нельзя показать безусловным успехом.
3. **Осмотр не является побочным продуктом отказа.** Раскрытие деталей рендерилось
   внутри блока восстановительных действий, поэтому обычный обработанный документ —
   подавляющее большинство — открыть было нельзя. Раскрытие живёт в самой строке и
   не зависит от `actions`.
   **Контроль раскрытия — само имя файла** (`.a3-drow-file-link`, изменение
   контракта 16.09.2026): подчёркнутая кнопка с `aria-expanded`/`aria-controls`
   вместо отдельной кнопки «Vorschau» в собственной колонке. Отдельная кнопка
   называла словом то, чем уже является имя документа, и занимала колонку, из-за
   которой имя переносилось по слогам. Доступное имя контрола остаётся
   `ds.documentRow.actionOn` («Vorschau · <файл>»), поэтому для скринридера
   действие названо по-прежнему. Строка без `onToggleDetail` рендерит имя как
   `<b>` — некликабельный текст не притворяется контролом.
4. **`layout="rail"` — настоящая компоновка** (`.a3-pjob-rail`, `.a3-pjob-summary`):
   то же задание узкой контекстной колонкой рядом с реестром, который оно описывает.
   Те же состояния, те же счётчики, та же семантика прогресса, без списка строк.
...
**Закрывает:** DC-38 (иерархия метрик) · R-11 · R-18 · DATA-001 · SCOPE-001
(вместе с `CommercialNumber` — правило 7; вместе с `SemanticStatus` — правило 8)

**Зачем существует.** Сводка коммерческого результата Option собиралась
раздельно на пяти поверхностях: карточка сохранённой Option, коммерческий
рельс, «Finale Prüfung», страница деталей затрат и вход клиентской проекции.
Четыре из них вообще не могли показать `€/m² WFL nach WoFlV` или
`€/m² NUF nach DIN 277`, а доказать согласие между ними было нечем. Контракт
поэтому принимает **один объект проекции**, а не набор числовых пропов:
`projection: OptionCommercialProjection`. Пять вызывающих — это пять шансов
передать ставку под чужой подписью; один объект такого шанса не даёт.

**Анатомия:**
`div.a3-oms[data-result-version]` (`.a3-oms-card` — плотность карточки,
`.a3-oms-rail` — плотность рельса и обзора) →
`div.a3-oms-total` → `CommercialNumber.a3-oms-total-value` +
`span.a3-oms-total-label` (подпись покрытия, выведенная результатом, R-18) ·
`dl.a3-oms-facts` → `div.a3-oms-fact[data-metric]` →
`dt.a3-oms-fact-k` (+ `span.a3-oms-fact-denominator` — норматив принадлежит
ИМЕНИ метрики, правило 31) + `dd.a3-oms-fact-v.numeric` ·
ось сертификации/энергии → `dd.a3-oms-fact-v` + `span.a3-oms-fact-note` ·
неизвестный знаменатель → `dd.a3-oms-fact-v` → `SemanticStatus[tone="unknown"]` ·
`p.a3-oms-note` — утверждение о неаддитивности.

**Варианты:** `card` (карточка Option: caption-плотность) ·
`rail` (коммерческий рельс и обзор: body-плотность значений).
Вариант — явное перечисление, не набор булевых пропов.

**Состояния:**

*Взаимодействие*: `default` — сводка не является контролом; ни одна её часть
не принимает ввод, поэтому `hover`/`focus`/`pressed`/`selected`/`disabled`
контрактом не объявляются (маршруты к решениям принадлежат окружающей
поверхности).

*Данные*: `ready` (есть результат и хотя бы один применимый знаменатель) ·
`partial` (сегмент применим, а его площадь неизвестна → строка `gaps`
сохраняет норматив на экране и НИКОГДА не печатает `0`; подпись итога при
этом — `Zwischensumme der kalkulierten Positionen`, SCOPE-001 и правило 16) ·
`loading` — notApplicableReason: проекция выводится синхронно из
`CommercialResult`, который владелец уже держит; собственного чтения за
сводкой нет ·
`error` — notApplicableReason: сбой вывода результата — состояние
`commercialSnapshot`, и его показывает поверхность вокруг сводки, а не вторая
ошибка поверх ·
`stale` — notApplicableReason: устаревание объявляет `CommercialResult.trust`;
сводка устаревшего результата — правдивая сводка результата, который сам об
этом говорит ·
`empty` — notApplicableReason: Option без выводимого результата не рендерит
сводку вовсе — владелец показывает названное отсутствие ·
`permission` — notApplicableReason: видимость решает профиль выдачи (OUT-*) до
рендера.

**OMS-001. Знаменатель назван всегда.** Каждая величина приходит из
`Rate` с ТИПИЗИРОВАННЫМ знаменателем, а слова берутся из типа
(`DENOMINATOR_LABEL`). Голая `€/m²` невыразима: в контракте нет пропа, из
которого её можно было бы собрать.

**OMS-002. Смешанного знаменателя не существует.** `Σ WFL + Σ NUF` запрещена
(правило 39, R-11, DATA-001), и запрет структурный: проекция не имеет поля,
способного удержать такую сумму. При смешанном использовании сводка показывает ДВЕ
сегментные метрики, каждая под своим нормативом.

**OMS-003. Две сегментные метрики не складываются, и это сказано.** Когда
сегментных метрик больше одной, `metricsAreAdditive === false`, и сводка
печатает утверждение о неаддитивности из проекции — не по памяти каждой
поверхности.

**OMS-004. Масштаб не конкурирует с ведущей метрикой.** `€/m² BGF oberirdisch`
объявляется ролью `scale` и идёт последним: это масштаб стройки для всего
комплекса (правило 39), а не Leitkennzahl сегмента.

**OMS-005. Согласие доказуемо, а не заявлено.** `data-result-version` несёт
`CommercialResult.version`, поэтому две поверхности сравнимы механически.

**OMS-006. Слова приходят пропом.** `labels` обязателен: возможность не читает
словарь продукта, иначе её нельзя отрендерить в витрине (D-28).

**Токены:** `--space-1/-2/-3` · `--color-text-primary/-secondary` ·
`--font-weight-bold` · `--font-numeric` · `--type-caption-size/-line` ·
`--type-small-size/-line` · `--type-body-size/-line` · `--measure-band`.

**Запреты:** числовые пропы вместо проекции · собственная арифметика ставки
или итога · подпись итога, собранная на месте (только `totalLabel` результата,
R-18) · `0` вместо неизвестного знаменателя · оранжевый на канве (правило 5) ·
собственные ключи i18n и чтение словаря внутри возможности · фиксированная
ширина текстового контейнера (правило 36).
